...
class FilenameExtractor {
    ...
    fun extract(url: String, contentDisposition: String?, mimeType: String?, directory: File): FilenameExtractionResult {  //changed parameters
        ...
        while (evaluateGuessQuality(guesses, pathSegments) != GuessQuality.TriedAllOptions) {
        ...
        return bestGuess(guesses, directory)
    }
    ...
    private fun evaluateGuessQuality(
        ...
        if (pathSegments.size < 2) return GuessQuality.TriedAllOptions

        return GuessQuality.NotGoodEnough
    }
    ...
    fun handleDuplicates(filename: String, directory: File): String {  //removed private to make it separately available
